...
exports.deleteMessage = exports.updateMessage = exports.addMessage = exports.getMessage = exports.getMessages = exports.Principal = void 0;
...
var SHA2 = class extends Hash {
    ...
    digestInto(out) {
        ...
        for(let i12 = pos; i12 < blockLen; i12++)buffer[i12] = 0;
        ...
        for(let i11 = 0; i11 < outLen; i11++)oview.setUint32(4 * i11, state[i11], isLE2);
    }
    digest() {
...
var SHA256 = class extends SHA2 {
    ...
    process(view, offset) {
        for(let i15 = 0; i15 < 16; i15++, offset += 4)SHA256_W[i15] = view.getUint32(offset, false);
        for(let i13 = 16; i13 < 64; i13++){
            const W15 = SHA256_W[i13 - 15];
            const W2 = SHA256_W[i13 - 2];
            const s0 = rotr(W15, 7) ^ rotr(W15, 18) ^ W15 >>> 3;
            const s1 = rotr(W2, 17) ^ rotr(W2, 19) ^ W2 >>> 10;
            SHA256_W[i13] = s1 + SHA256_W[i13 - 7] + s0 + SHA256_W[i13 - 16] | 0;
        }
        let { A , B , C , D , E , F , G , H  } = this;
        for(let i14 = 0; i14 < 64; i14++){
            const sigma1 = rotr(E, 6) ^ rotr(E, 11) ^ rotr(E, 25);
            const T1 = H + sigma1 + Chi(E, F, G) + SHA256_K[i14] + SHA256_W[i14] | 0;
...
var Principal = class {
    ...
    compareTo(other) {
        for(let i16 = 0; i16 < Math.min(this._arr.length, other._arr.length); i16++){
            if (this._arr[i16] < other._arr[i16]) return "lt";
            else if (this._arr[i16] > other._arr[i16]) return "gt";
        }
...
var v4_default = v4;
// src/index.ts
var messageStorage = new StableBTreeMap(0, 44, 1024);
...
function updateMessage(id, payload) {
    return match(messageStorage.get(id), {
        Some: (message)=>{
            const updatedMessage = _objectSpread({}, message, payload, {
...
globalThis.crypto = {
    getRandomValues: ()=>{
        ...
        for(let i18 = 0; i18 < array.length; i18++){
            array[i18] = Math.floor(Math.random() * 256);
        }
        return array;
    }
};
